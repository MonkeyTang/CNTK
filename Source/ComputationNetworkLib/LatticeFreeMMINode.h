...
public:
    ...
    LatticeFreeMMINode(DEVICEID_TYPE deviceId, const wstring& name, const wstring& fstFilePath, const wstring& smapFilePath, const ElemType acweight, const bool usePrior, const int alignmentWindow, const ElemType ceweight, const bool useSenoneLM, const wstring& transFilePath)
    ...
    LatticeFreeMMINode(const ScriptableObjects::IConfigRecordPtr configp)
        : LatticeFreeMMINode(configp->Get(L"deviceId"), L"<placeholder>", configp->Get(L"fstFilePath"), configp->Get(L"smapFilePath"), configp->Get(L"acweight"), configp->Get(L"usePrior"), configp->Get(L"alignmentWindow"), configp->Get(L"ceweight"), configp->Get(L"useSenoneLM"), configp->Get(L"transFilePath"))
    {
    ...
    virtual void BackpropToNonLooping(size_t inputIndex) override
    {
        if (inputIndex == 1)
        {
            ...
            {
                // (1-alpha) * r_DEN + alpha * P_net - r_NUM
                m_softmax->InplaceExp();
                Matrix<ElemType>::ScaleAndAdd(m_ceweight, *m_softmax, 1 - m_ceweight, *m_posteriorsDen);
                Matrix<ElemType>::AddScaledDifference(Gradient(), *m_posteriorsDen, *m_posteriorsNum, gradient);
            }
        }
    }
